...
const AddToken = () => {
  ...
  const [AddTokenTranactions, setAddTokenTransaction] = useState([]);
  ...
  useEffect(() => {
    // const fetchAllAddTokenTransaction = async () => {
    //   const fullUrl = `${base_url}/admin/transactions/add`;
    ...
    //     console.log(
    //       "data from all decryptedDataAllAddTokenTransactionList",
    //       data
    //     );
    //     const decryptedDataAllAddTokenTransactionList = decryptData(data?.data);
    //     setAddTokenTransaction(decryptedDataAllAddTokenTransactionList); // Assuming the response is an array of profiles
    //     console.log(
    //       "decrypted Data listing for decryptedDataAllAddTokenTransactionList",
    //       decryptedDataAllAddTokenTransactionList
    //     );
    //   } catch (err) {
    ...
    // fetchAllAddTokenTransaction();

    setLoading(true);

    axios.get(`${base_url}/api/admin/transactions/add`, {
      headers: {
        Authorization: `Bearer ${token}` // Set the Authorization header with the token
      }
    }).then((data) => {
      console.log("transaction data", data);
      setAddTokenTransaction(data?.data.data)
      setLoading(false);
        ...
        <table className={styles.table}>
          ...
          <tbody className={styles.tableBody}>
            {loading
              ...
              : AddTokenTranactions?.map((item, idx) => (
                <tr key={item?._id}>
                  <td>{idx + 1}</td>
